...
const Profile: FC<Props> = ({ user }) => {
  ...
  // Use useEffect for scroll event listener instead of direct window access
  useEffect(() => {
    const handleScroll = () => {
  ...
  useEffect(() => {
    if (coursesData && user.courses) {
      const filteredCourses = user.courses
        .map((userCourse: any) =>
          coursesData.courses.find((course: any) => course._id === userCourse._id)
        )
        .filter((course: any) => course !== undefined);
      setCourses(filteredCourses);
    }
  }, [coursesData, user.courses]); // Added user.courses as dependency
